...
/**
 * XL: TOP3 property: When >50% of the population are the top 3 names in decade D, it should classify as D.
 */
...
/**
 * TODO This is to implement an automated procedure for checking that: (but temporarily paused)
 *   When >50% of the population are the top 3 names in decade D, it should classify as D.
 * I have added the argument of 1950/1970/1990/2010.
 * It still needs to assign different ranges to those names, and then return SAT accordingly, just like robustness.
 *
 * 1950s: (1) Mary, 27.08%	(2) Linda, 24.42%	(3) Patricia, 19.89%. -- 71.39%
 * 1970s: (1) Jennifer, 28.48%	(2) Amy, 13.17%	(3) Melissa, 12.40%. -- 54.05%
 * 1990s: (1) Jessica, 17.39%	(2) Ashley, 17.32%	(3) Emily, 13.61%. -- 48.32%
 * 2010s: (1) Emma, 17.74%	(2) Sophia, 17.64%	(3) Olivia, 16.46%. -- 51.84%
 *
 * @param decade
 * @param targetOutputVariableIndex
 * @param otherOutputVariableIndex
 * @return true if SAT (i.e., found a CEX)
 */
bool top3_main(int decade, unsigned targetOutputVariableIndex, unsigned otherOutputVariableIndex, char *finalOutputFile) {
    printf("%d\n", decade);

    String networkPath = FULL_NET_PATH;

    printf( "Checking output index: %u\n", targetOutputVariableIndex);
    ...
    Reluplex reluplex( inputLayerSize + ( 3 * numReluNodes ) + ( 2 * outputLayerSize ) +
                       1 + 1 + 1,  // XL: add one more variable for "input sum to 1.0 constraint"
                       finalOutputFile,
                       networkPath );
    ...
    // XL: Input Constraint: all inputs sum to 1.0
    unsigned inputConstraintVar = newIndex;
    ++newIndex;
    ...
    try
    {
        ...
        // FIXME If saying only Mary/Linda has high percentage, then it may not get verified..
        int num_names = 16;
        int idx_mary = 8;
        int idx_linda = 7;
        for (int i = 0; i < num_names; i++) {
            if (i == idx_mary || i == idx_linda) {
                reluplex.setLowerBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0.4, neuralNetwork ) );
                reluplex.setUpperBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 1.0, neuralNetwork ) );
            }
                /*
                if (i == idx_linda) {
                    reluplex.setLowerBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0.8, neuralNetwork ) );
                    reluplex.setUpperBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 1.0, neuralNetwork ) );
                }
                */
            else {
...
int main( int argc, char **argv )
{
    ...
    if (argc < 2) {
        printf("Error! Do need to provide one arg indicating the decade to check.\n");
        exit(1);
    }

    int decade = atoi(argv[1]);

    // This is the contender, search for bad cases where target > other
    unsigned otherOutputVariableIndex;
    switch (decade) {
        case 1950:
            otherOutputVariableIndex = 0;
            break;
        case 1970:
            otherOutputVariableIndex = 1;
            break;
        case 1990:
            otherOutputVariableIndex = 2;
            break;
        case 2010:
            otherOutputVariableIndex = 3;
            break;

        default:
            printf("Error! Experiment only supports 1950, 1970, 1990, 2010 as decade.\n");
            exit(1);
    }
    ...
    for (unsigned i = 0; i < 4; i++) {
        // i is targetOutputVariableIndex, checking that every OTHER index has smaller score than category K.
        if (i != otherOutputVariableIndex) {
            printf( "Testing for category %u.\n", i);
            sat = top3_main(decade, i, otherOutputVariableIndex, finalOutputFile);
            printf( "Test for category %u DONE. Result = %s\n", i, sat ? "SAT" : "UNSAT" );
            printf( "\n\n" );
        }
    ...
    printf("---XL: Eventually, for decade %d, the result of finding CEX is %d.\n", decade, sat);
    return 0;
}
