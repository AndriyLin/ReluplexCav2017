...
int main( int argc, char **argv )
{
    ...
    // XL: Experiment goal: when only male terms appear, check the Male prediction is maximal, i.e., Female prediction is minimal.
    // The "minimality" is already in this code (copied from prop5).
    unsigned targetOutputVariableIndex = 0;  // XL: female prediction score
    // This is the contender
    unsigned otherOutputVariableIndex = 1;  // XL: male prediction score

    if ( argc < 2 )
    {
        printf( "Error! Checking whether 'only male' or 'only female'.\n" );
        exit( 1 );
    }

    int checking_male = atoi( argv[1] );
    if (checking_male == 1) {
        // so male prediction should be larger, female prediction should be minimal
        targetOutputVariableIndex = 0;
        otherOutputVariableIndex = 1;
    }
    else if (checking_male == 0) {
        // so female prediction should be larger, male prediction should be minimal
        targetOutputVariableIndex = 1;
        otherOutputVariableIndex = 0;
    }
    else {
        printf( "Error! Checking whether 'only male' (1) or 'only female' (0).\n" );
        exit( 1 );
    }
    ...
    try
    {
        ...
        // XL: set input bounds as input property -- only allowing male terms to occur
        int gender_terms_size = 20;  // per gender
        int allowed_max = 1;
        for (int i = 0; i < gender_terms_size; i++) {
            reluplex.setLowerBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0, neuralNetwork ) );
            if (checking_male) {
                reluplex.setUpperBound( nodeToVars[Index(0, i, true)], normalizeInput( i, allowed_max, neuralNetwork ) );
            }
            else {
                reluplex.setUpperBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0, neuralNetwork ) );
            }
        }
        for (int i = gender_terms_size; i < gender_terms_size * 2; i++) {
            reluplex.setLowerBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0, neuralNetwork ) );
            if (checking_male) {
                reluplex.setUpperBound( nodeToVars[Index(0, i, true)], normalizeInput( i, 0, neuralNetwork ) );
            }
